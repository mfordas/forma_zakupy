...
router.post('/', async (req, res) => {
  ...
  router.put('/:id/product', async (req, res) => {
    ...
    const shoppingListHandler = await ShoppingList.findById(req.params.id, 'products', { lean: true });
  ...
  router.delete('/:id/product/:idProduct', async (req, res) => {
    ...
    const shoppingListHandler = await ShoppingList.findById(req.params.id, 'products', { lean: true });
    const filteredProducts = await shoppingListHandler.products.filter(el => el._id.toString() !== req.params.idProduct);
  ...
  router.put('/:id/product/:idProduct', async (req, res) => {
    ...
    if (!product) return res.status(404).send('Nie znaleziono produktu z takim ID.');
